...
class MainWindow(QMainWindow):
    ...
    def on_choose_library(self) -> None:
        ...
        if not path:
            return
        try:
            resolved_path = ProjectLibrary.resolve_library_root(path)
            self.pm.set_library(resolved_path)
            self.statusBar().showMessage(f"Library: {resolved_path}")
            self._refresh_project_list()
        except ProjectLibraryError as e:
            _err(self, str(e))
    ...
    def on_delete_selected(self) -> None:
        ...
        if not _confirm(self, "Delete this project from the library? This cannot be undone."):
            return
        try:
            self.pm.delete(pid, remove_file=True)
